...
/* Create a hash table.  The argument hash is a function that returns
   a an integer when applied to an element of the hash table.  The
   argument equal() is a function that returns a true value if object
   arguments are considered equal, 0 otherwise. */

extern hash_table
hash_create(int (*hash) (void *), int (*equal) (void *o1, void *o2));
...
/* The hash table has the notion of a cursor.  This function positions
   the cursor to an object equal() to the one given as argument, if
   such an object exists.  In that case a true value is returned.  If
   no object equal() to the one given exists in the table, a 0 is
   returned, and the cursor is positioned to a place where insertion
   of the new object should take place.  A typical call would be
   	
		if(!hash_position(t, o)) hash_insert(t, o);
   
   that is, first position to the place where object o ought to be
   located.  If it is not in the table, then cursor is positioned
   where it should be inserted, thus you can immediately insert it. */

extern int hash_position(hash_table ht, void *object);
...
extern void hash_insert(hash_table, void *object);
