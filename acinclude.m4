...
dnl @version $Id: acinclude.m4,v 1.1.1.1.2.1 2003/06/11 20:09:17 yoann Exp $
...
AC_ARG_WITH(DOWN-exec-prefix,[  --with-]DOWN[-exec-prefix=PFX Exec prefix where $1 is installed (optional)],
  ...
  if test "$UP[]_CONFIG" = "no" ; then
	...
	if test -z $DOWN[]_wanted_micro_version; then 
		DOWN[]_wanted_micro_version="0"
	fi

	if test -z $DOWN[]_config_micro_version; then
        ...
        if test "$DOWN[]_config_major_version" -lt "$DOWN[]_wanted_major_version" \
          -o \( "$DOWN[]_config_major_version" -eq "$DOWN[]_wanted_major_version" \
          -a    "$DOWN[]_config_minor_version" -lt "$DOWN[]_wanted_minor_version" \) \
          -o \( "$DOWN[]_config_major_version" -eq "$DOWN[]_wanted_major_version" \
          -a    "$DOWN[]_config_minor_version" -eq "$DOWN[]_wanted_minor_version" \
          -a    "$DOWN[]_config_micro_version" -lt "$DOWN[]_wanted_micro_version" \) ; then
