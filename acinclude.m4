...
dnl @version $Id: acinclude.m4,v 1.1 2002/03/22 13:46:29 turpeau Exp $
...
dnl @synopsis AC_PATH_GENERIC2(LIBRARY [, MINIMUM-VERSION [, ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND]]])
dnl
dnl Runs a LIBRARY-config script and defines LIBRARY_CFLAGS and LIBRARY_LIBS
dnl
dnl The script must support `--cflags' and `--libs' args.
...
dnl For example:
dnl
dnl    AC_PATH_GENERIC2(Foo, 1.0)
dnl
dnl would run `foo-config --version' and check that it is at least 1.0
dnl
dnl If so, the following would then be defined:
dnl
dnl    FOO_CFLAGS to `foo-config --cflags`
     ...
     ifelse([$2], , ,[
        ...
        if test "$DOWN[]_config_major_version" -lt \
          ...
          -o \( "$DOWN[]_config_major_version" -eq \
                        "$DOWN[]_wanted_major_version" \
            -a "$DOWN[]_config_minor_version" -eq \
                        "$DOWN[]_wanted_minor_version" \) ; then
          # older version found
          no_[]DOWN=yes
          echo -n "*** An old version of $1 "
          echo -n "($DOWN[]_config_major_version"
          echo    ".$DOWN[]_config_minor_version"
   	  echo -n "*** You need a version of $1 newer than "
          echo -n "$DOWN[]_wanted_major_version"
          echo    ".$DOWN[]_wanted_minor_version"
          echo "***"
          echo "*** If you have already installed a sufficiently new version, this error"
          echo "*** probably means that the wrong copy of the DOWN-config shell script is"
          echo "*** being found. The easiest way to fix this is to remove the old version"
